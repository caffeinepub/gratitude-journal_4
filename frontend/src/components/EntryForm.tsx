...
export default function EntryForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Stop listening if active and append final transcript
      if (isListening) {
        const finalTranscript = stopListening();
        if (finalTranscript) {
          const fullText = text ? `${text} ${finalTranscript}` : finalTranscript;
          
          // Create entry with full text
          const entryId = await createEntry.mutateAsync(fullText.trim());
